...
export class PbLibraryComponent implements OnInit {
  ...
  constructor(
    private pbLibraryService: PbLibraryService
  ) {
    this.getUsersList();
    this.getNrTypes();
  }
  ...
  public viewContact(contact : any){
    this.pbLibraryService.getCurrentUser(contact.id).subscribe((data: any): void => {
  ...
  public removeContact(contact : any){
    this.pbLibraryService.removeCurrentUser(contact.id).subscribe((data: any): void => {
      this.getUsersList();
      this.currentUser = '';
    });
  }
  ...
  public saveEdits(contact: any){
    this.pbLibraryService.saveEditedUser(contact.id, contact).subscribe((data: any): void => {
      this.getUsersList();
  ...
  public saveAddedUser(contact: any){
    this.pbLibraryService.saveAddedUser(contact).subscribe((data: any): void => {
      this.getUsersList();
      this.editcontact = false;
      this.addcontact = false;
      this.viewContact(contact);
  ...
  public removePhoneNumberForUser(contact:any, number:any){
    ...
    this.pbLibraryService.removePhoneNumberForUser(contact.id, contact ).subscribe((data: any): void => {
    });
  }
